...
def small_change(
    variants: typing.Sequence[Rows.Core.Item],
) -> typing.Callable[[], Rows.Core.Item]:
    i = -1
...
@pytest.mark.benchmark(group="rows")
def test_set_small_change(
    benchmark: pytest_benchmark.plugin.BenchmarkFixture,
    rows: Rows.Core,
    row: Rows.Core.Item,
):
    initial = dataclasses.replace(
        row, metadata=row.metadata | {Item.Metadata.Key("i"): 1}
    )
