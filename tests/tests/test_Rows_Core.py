...
def test_extend_metadata_enum(rows: Rows.Core, row: Rows.Core.Item):
    metadata_1 = row.metadata | {
        Item.Metadata.Key("new_column"): Item.Metadata.Enumerable("lalala")
    }
    metadata_2 = row.metadata | {
        Item.Metadata.Key("new_column"): Item.Metadata.Enumerable("lololo")
    }
...
def test_none_metadata_enum_value(rows: Rows.Core, row: Rows.Core.Item):
    ...
    metadata_2 = row.metadata | {
        Item.Metadata.Key("new_column"): Item.Metadata.Enumerable(None)
    }
...
def test_get_exact(
    rows: Rows.Core, row: Rows.Core.Item, query_all: Query, changed_row: Rows.Core.Item
):
    rows.append(row)
    rows.append(changed_row)
